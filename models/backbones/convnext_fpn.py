...
class ConvNeXtFPN(nn.Module):
    ...
    def __init__(
        ...
        model_name='tiny', 
        pretrained=True, 
        feature_channels=None,  # ✅ 파라미터 추가 (호환성 위해)
        fpn_channels=256, 
        use_fpn=True
    ):
        super().__init__()
        ...
        # --- [수정] 모델 이름을 비교하기 전에 하이픈을 언더스코어로 변경 ---
        normalized_model_name = model_name.replace('-', '_')

        # ConvNeXt 모델 선택 및 stage_channels 정의
        if normalized_model_name == 'convnext_tiny':
            self.convnext = convnext_tiny(weights='DEFAULT' if pretrained else None)
            self.stage_channels = feature_channels or [96, 192, 384, 768]
            self.extract_layers = [1, 3, 5, 7]
            
        elif normalized_model_name == 'convnext_small':
            self.convnext = convnext_small(weights='DEFAULT' if pretrained else None)
            self.stage_channels = feature_channels or [96, 192, 384, 768]
            self.extract_layers = [1, 3, 5, 7]
            
        elif normalized_model_name == 'convnext_base':
            self.convnext = convnext_base(weights='DEFAULT' if pretrained else None)
            self.stage_channels = feature_channels or [128, 256, 512, 1024]
            self.extract_layers = [1, 3, 5, 7]
        else:
            raise ValueError(f"Unsupported model: {model_name}")
        
        # feature_channels 검증 (옵션)
        if feature_channels and len(feature_channels) != 4:
            raise ValueError(f"feature_channels must have 4 elements, got {len(feature_channels)}")
    ...
    def forward(self, x):
        ...
        # Dict 형태로 반환 (Mask2Former 호환성)
        feature_dict = {}
        for i, feat in enumerate(fpn_features):
            # P2, P3, P4, P5 형태로 저장
            feature_dict[f'P{i+2}'] = feat
        
        return feature_dict  # ✅ Dict 반환으로 수정
